...
class Agente:
    ...
    def detectar_bug(self, canos, chao, bug_rect):
        ...
        if bug_ativo and bug_rect.collidepoint(self.x, self.y):
            logging.warning(f"Bug detectado pelo agente na posição: ({self.x}, {self.y})")
            self.reward -= 1
    ...
    def desenhar(self, tela):
        ...
        self.imagem_rotacionada = pygame.transform.rotate(self.imagem, self.angulo)
        apos_centro_imagem = self.imagem.get_rect(topleft=(self.x, self.y)).center
        self.retangulo =self.imagem_rotacionada.get_rect(center=apos_centro_imagem)
        tela.blit(self.imagem_rotacionada, self.retangulo.topleft)
