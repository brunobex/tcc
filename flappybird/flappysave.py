...
def main(genomas, config):  # função de fitness
    global geracao, velocidade_jogo
    geracao += 1

    genome_saved = False  # Variável para controlar se o genoma foi salvo
    ...
    while rodando:
        ...
        botao_rect = desenhar_tela(tela, passaros, canos, chao, pontos)

        # Verifica se algum pássaro atingiu 20 pontos e salva o genoma
        if pontos >= 20 and not genome_saved:
            save_genome(lista_genomas[0])
            genome_saved = True
...
def rodar(caminho_config):
    ...
    if ai_jogando:
        # Carregar um genoma salvo, se disponível
        if os.path.exists('best_genome.pkl'):
            print("Carregando genoma salvo...")
            genoma = load_genome('best_genome.pkl')
            main([(1, genoma)], config)
        else:
